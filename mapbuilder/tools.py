...
def binary_split(i, j, W, H):
    ...
    representatives = [(c_i - Hc / 2, c_j - Wc / 2),
                          (c_i + Hf / 2, c_j - Wc / 2),
                          (c_i - Hc / 2, c_j + Wf / 2),
                          (c_i + Hf / 2, c_j + Wf / 2)]
    sizes = [(Hc, Wc), (Hf, Wc), (Hc, Wf), (Hf, Wf)]
...
def generate_windows(window_size: int, initial_resolution: int, resolution: int = 1024):
    ...
    border_indexes = [(-1,-1), (-1, resolution), (resolution, -1), (resolution, resolution)]  ## four corners
    for i in range(initial_resolution):
        p = (i * window_size + window_size / 2 - 0.5)
        border_indexes += [(-1, p), (p, -1), (p, resolution), (resolution, p)]  ## four borders
    ...
    indexes += [(i * window_size + window_size / 2 - 0.5, p) for i in range(initial_resolution)]  ## right border
    ...
    sizes += [((resolution - initial_resolution * window_size), window_size)] * initial_resolution     ## bottom border
    
    indexes += [(p, p)]  ## bottom right corner
