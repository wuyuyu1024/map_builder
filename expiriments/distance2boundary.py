...
for data_name, dataset in datasets_real.items():
    ...
    for ppinv_name, ppinv in PPinv_dict.items():
        ...
        np.savez(f'{save_dir}/{data_name}_{ppinv_name}_dist2boundary.npz', label_gt=label_gt, dist_gt=dist_gt, label_fast=label_fast, dist_fast=dist_fast, sparse=sparse, time_gt=time_gt, time_fast=time_fast, grid=GRID, X2d=X2d)