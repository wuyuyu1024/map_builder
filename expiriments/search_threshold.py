...
# GRID = 256
grid_list = [100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800, 1900, 2000]
# grid_list = [256]
...
for ppinv_name, ppinv in PPinv_dict.items():
    ...
    for GRID in grid_list:
        ...
        for t in tqdm(threshold_list):
            ...
            results = results._append({'Data': data_name, 'PPinv': ppinv_name, 'time near dummy': time_near_gt, 'time near': time_near, 'time grad dummy': time_grad_gt, 'time grad': time_grad, 'time dist dummy': time_dist_gt, 'time dist': time_dist, 'threshold': t, 'error near abs': error_near_abs, 'error near sq': error__near_sq, 'error grad abs': error_grad_abs, 'error grad sq': error_grad_sq, 'error dist abs': error_dist_abs, 'error dist sq': error_dist_sq, 'GRID':GRID, 'num_sparse_near': len(sparse_near), 'num_sparse_grad': len(sparse_grad), 'num_sparse_dist': len(sparse_dist)}, ignore_index=True)

            results.to_csv(f'{save_dir}/new_varygrid_dist_t02_{date}.csv', index=False)
